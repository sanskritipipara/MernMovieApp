const Movies = require("../models/Movies");
const formidable = require("formidable");
const _ = require("lodash");
const fs = require("fs");

exports.getMovieById = (req, res, next, id) => {
    Movies.findById(id)
    .exec((err, movie) => {
        if(err) {
            return res.status(400).json({
                error: "Movie not found"
            });
        }
        req.movie = movie;
        next();
    });
};

exports.createMovie = (req, res) => {
    let form = new formidable.IncomingForm();
    form.keepExtensions = true;

    form.parse(req, (err, fields, file) => {
        if(err) {
            return res.status(400).json({
                error : "problem with image"
            });
        }
        //destructure the fields
        const {title, description, release_date, vote_average, runtime, vote_count, status, popularity} = fields;

        if( !title || !description || !release_date || !vote_average || !runtime || !vote_count || !status || !popularity)
        {
            return res.status(400).json({
                error: "Please include all fields"
            });
        }

        //handling field
        let movie = new Movies(fields)

        //handle file here
        if(file.photo) {
            if(file.photo.size > 3000000){
                return res.status(400).json({
                    error: "File size too big"
                });
            }
            movie.photo.data = fs.readFileSync(file.photo.path)
            movie.photo.contentType = file.photo.type ;
        }
        movie.save((err, movie) => {
            if(err){
                return res.status(400).json({
                    error: "Saving movie in DB failed"
                });
            }
            res.json(movie);
        });
    });
};

exports.getMovie = (req, res) => {
    req.movie.photo = undefined;
    return res.json(req.movie);
};

exports.photo = (req, res, next) => {
    if(req.movie.photo.data){
        res.set("Content-Type", req.movie.photo.contentType);
        return res.send(req.movie.photo.data);
    }
    next();
};

exports.deleteMovie = (req, res) => {
   let movie = req.movie;
   movie.remove((err, deletedMovie) => {
       if(err) {
           return res.status(400).json({
               error : "Failed to delete the movie"
           });
       }
       res.json({
           message : "Deletion was a success",
           deletedMovie
       });
   });
};

exports.updateMovie = (req, res) => {
    let form = new formidable.IncomingForm();
    form.keepExtensions = true;

    form.parse(req, (err, fields, file) => {
        if(err) {
            return res.status(400).json({
                error : "problem with image"
            });
        }
       
        //updation code
        let movie = req.movie;
        movie = _.extend(movie, fields);         //lodash used

        //handle file here
        if(file.photo) {
            if(file.photo.size > 3000000){
                return res.status(400).json({
                    error: "File size too big"
                });
            }
            movie.photo.data = fs.readFileSync(file.photo.path)
            movie.photo.contentType = file.photo.type ;
        }
        movie.save((err, movie) => {
            if(err){
                return res.status(400).json({
                    error: "Updation of movie failed"
                });
            }
            res.json(movie);
        });
    });
};

exports.getAllMovies = (req, res) => {
    let limit = req.query.limit ? parseInt(req.query.limit) : 8
    let sortBy = req.query.sortBy ? req.query.sortBy : "_id";

    Movies.find()
    .select("-photo")
    // .populate("category")
    .sort([[sortBy, "asc"]])
    .limit(limit)
    .exec((err, movies) => {
        if(err){
            return res.status(400).json({
                error: "No movie found"
            });
        }
        res.json(movies);
    });
};

// exports. getAllUniqueCategories = (req, res) => {
//     Product.distinct("category", {}, (err, category) => {
//         if(err){
//             return res.status(400).json({
//                 error: "NO category found"
//             });
//         } 
//         res.json(category);
//     });
// };

exports.updateStock = (req, res, next) => {

    let myOperations = req.body.order.products.map(prod => {
        return {
            updateOne : {
                filter : { _id : prod._id},
                update: {$inc: {stock: -prod.count , sold: +prod.count}}
            }
        };
    })
    Product.bulkWrite(myOperations, {}, (err, products) => {
        if(err){
            return res.status(400).json({
                error: "Bulk Operation failed"
            });
        }
        next();
    });
};

